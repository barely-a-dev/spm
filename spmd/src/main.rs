    ...
    let matches = ClapCommand::new("SPM")
        ...
        .arg(
            Arg::new("mpack-exes")
                .short('e')
                .long("mpack-exes")
                .help("Package every file in IN_DIR into a package, OUTPUT")
                .num_args(2)
                .value_names(["IN_DIR", "OUTPUT"])
        )
        .arg(
            Arg::new("mirror-repo")
                .long("mirror")
        ...
        .arg(
            Arg::new("custom-name")
                .short('n')
                .long("name")
                .help("Use this name during package creation instead of using the name in the filename or the filename")
                .num_args(1)
                .value_name("VERSION")
        )
        .arg(
